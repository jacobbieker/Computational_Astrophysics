...
def hybrid_gravity(N,
    ...
    if not heavy_flag:
        combined_gravity = light_gravity
    elif not light_flag:
        combined_gravity = heavy_gravity
    ...
    while current_time < t_end:
        ...
        R_halfmass = LagrangianRadii(combined_gravity.particles, massf=MassFraction, verbose=False)[
            0]  # Both the Halfmass radius and the dynamical time scale of the cluster
    ...
    p4, = ax2.plot(time_list.value_in(units.Myr), Rhm.value_in(units.parsec), ls='-.', label='Halfmass Radius (t)',
                   color='g')
    ax1.semilogy()
    ...
    matplotlib.pyplot.title('Combined Gravity, IMF=%s, Gravity type=%s\nN=%i, Mcut= %.1f, Opening angle=%.1f' % (
    imf, method, N, m_cut.value_in(units.MSun), theta))
    fig.tight_layout()
    matplotlib.pyplot.savefig(
        './results/cluster_%s_%s_N=%i_mcut=%.1f_theta=%.1f.pdf' % (imf, method, N, m_cut.value_in(units.MSun), theta))
